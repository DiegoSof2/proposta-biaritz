...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      if (pathname === "/auth/login") return await authLogin(request, env, url);
      if (pathname === "/auth/callback") return await authCallback(request, env, url);
      if (pathname === "/auth/me") return await authMe(request, env);
      if (pathname === "/auth/logout") return authLogout();
      if (pathname === "/api/publish") return await apiPublish(request, env);
...
async function apiPublish(request, env) {
  ...
  treeItems.push({ path: "products.json", mode: "100644", type: "blob", sha: jsonBlob.sha });

  for (const f of fotos) {
    if (!f || !f.path || !f.base64) continue;
    const path = f.path.replace(/^\/+/, "").replace(/^site\//, "");
